...
export function BrandForm() {
    ...
    if (isEditMode) {
      const fetchBrand = async () => {
        try {
          ...
          showToast("Error", "Failed to load brand", "error");
        }
      };
      fetchBrand();
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!name.trim()) {
      setErrors({ name: "Brand Name is physically required" });
      return;
    }

    try {
      setLoading(true);
      const payload = { name, slug, origin, description, isActive };
      if (isEditMode) {
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 border-b-[3px] border-zinc-950 pb-6">
          <div>
            ...
            <h1 className="text-4xl font-black tracking-tighter uppercase whitespace-nowrap">
              {isEditMode ? 'Edit Brand_' : 'New Brand_'}
            </h1>
          </div>
        ...
        <div className="border-[3px] border-zinc-950 bg-white p-6 sm:p-8 flex flex-col gap-6">
          <div className="flex flex-col gap-2">
            <div className="flex justify-between items-center">
              <label className="text-[10px] font-black tracking-widest text-zinc-500 uppercase">Brand Name *</label>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="flex flex-col gap-2">
              <label className="text-[10px] font-black tracking-widest text-zinc-500 uppercase">Origin Country</label>
              <input 
                type="text" 
                value={origin}
                onChange={e => setOrigin(e.target.value)}
                placeholder="E.g. USA, Vietnam, Italy" 
                className="w-full border-[3px] border-zinc-950 px-4 py-3 font-bold text-sm uppercase focus:outline-none focus:bg-zinc-50"
              />
            </div>
          </div>
